...
\Cref{fig:reconfig_time_line_ex} show two example timelines of a good and a bad scenario when performing execution of multiple models in sequence.
% A frame enters the system at every $T$.
% Every new frame is to be processed by a new model.
% The \graicore{} is configured with model $A$ before time $0$.

% \begin{figure}[htbp]
%     \centering
%     \input{assets/reconfig_time_line_ex3}
%     \caption{
%         Example reconfiguration timeline.
%         $p_A$ and $c_B$ depicts the processing of model $A$ and configuration of model $B$ respectively.
%     }
